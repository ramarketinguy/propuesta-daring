...
export const onRequestGet: PagesFunction<Env> = async ({ params, env }) => {
  ...
  const order = await env.DB.prepare('SELECT id, order_code, external_reference, payment_id, status, shipping_status, tracking_number, admin_notes, mp_fee_cents, customer_name, customer_email, customer_phone, shipping_department, shipping_city, shipping_address, color, quantity, amount_cents, currency, created_at, updated_at, approved_at FROM orders WHERE id = ?').bind(id).first<Record<string, unknown>>();
